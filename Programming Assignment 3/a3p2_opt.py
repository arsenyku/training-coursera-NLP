...
class QMap:
    ...
    *****************************************************************************'''
    ...
    def Initialize(self, sourceLines, targetLines):
        ...
        for k in xrange(0,lineCount):
            ...
            sourceSentence = sourceLines[k].split()
            sourceSentence.insert(0, NULLTAG)
            
            targetSentence = targetLines[k].split()
            targetSentence.insert(0, NULLTAG)
            
            lk = len(targetSentence) - 1
            mk = len(sourceSentence) - 1
            ...
            if self.__qMap.has_key(key1):
                continue
            else:
                qList = dict()
                 
                for i_index in xrange(1, mk+1):
                    for j_index in xrange(0, lk+1):
                        qList[(j_index, i_index)] = default

                self.__qMap[key1] = qList
    ...
    def SetQValue(self, targetWordIndex, sourceWordIndex, targetSentenceLength, sourceSentenceLength, qValue):
        ...
        # Lengths do not include initial NULL character
    ...
    def GetQValue(self, targetWordIndex, sourceWordIndex, targetSentenceLength, sourceSentenceLength):
        ...
        if targetWordIndex > targetSentenceLength or \
            sourceWordIndex > sourceSentenceLength:
            
            print "Invalid parameters for GetQValue:", (j, i, lk, mk)
    ...
    def GetQValues(self, targetSentenceLength, sourceSentenceLength):
            ...
            for i in xrange(1, mk + 1):
                for j in xrange(0,lk + 1):
                    ...
                    if denominator == 0:
                        ...
                        self.__qMap.SetQValue(j, i, lk, mk, numerator/denominator)
...
#        return result
    ...
    def ComputeDelta(self, sentenceId, sourceWordIndex, targetWordIndex, sourceWords, targetWords):
        ...
        #print "ComputeDelta:", sentenceId, sourceWordIndex, targetWordIndex
        ...
        for j_index in xrange(0,lk+1):
            tValue = self._IBM1__tMap[a3p1.WordMap(fi, targetWords[j_index])]
            qValue = qList[j_index, i]
        ...
        #print "ComputeDelta:", numerator, denominator
        
        return numerator / denominator
        
        
    def LoadQValues(self, qFile):
        return self.__qMap.Load(qFile)
    
    
    
    def SaveQValues(self, qFile):
        return self.__qMap.Save(qFile)
...
#    aFile = "short.p2.out"
    ...
    tFile = "tValues.txt"
    newTFile = "tValues.p2opt.txt"
    qFile = "qValues.p2opt.txt"
